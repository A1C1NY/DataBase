...
导入、基础上游 API 获取和单进程定时采集，但不实现独立同步中心、缓存和生产化设施。
...
开发热重载时保持 `ENABLE_SCHEDULER=false`。需要演示定时采集时启用该开关，并使用单进程
`uvicorn app.main:app`，避免热重载重复启动任务。
...
4. 完成简单认证、乘客核心接口、收藏、API 调用和基础定时采集。
5. 完成四类简单分析和管理员接口。
...
- 定时采集仅支持固定坐标、单 Uvicorn 进程和简单失败日志。
- 不实现 Repository、异步 Session、缓存、重试队列、并发锁、自动清理和多进程防重。
- 密钥只放 `.env`，不得提交真实配置。
